...
const ALQueq = () => {
    ...
    const GenrateQR = async () => {
        ...
        let result = await api.ApiGenrateQR('R', am);
        if (result.error == 0) {
            SerQR(result.data);
        }
        ...
        QR
            ...
            <View style={{ backgroundColor: '#FF802C', flex: 1, alignItems: 'center' }}>
                <View style={{ padding: 10, marginBottom: 50, flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 30, color: '#fff', fontWeight: 'bold', flex: 1, textAlign: 'center' }}>Öde</Text>
                </View>
            ...
            :
            <SendMoney OnSubmited={(e: any) => { GenrateQR(); }} data={''} onChangeAmount={setAmount}></SendMoney>
    );
}
